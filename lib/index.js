...
module.exports = class WebpackAliOSSPlugin {
  ...
  apply (compiler) {
    ...
    compiler.plugin('after-emit', (compilation, cb) => {
      ...
      this.deleteFiles(compilation)
        .then((compilation) => this.uploadFiles(compilation))
        .then(() => { cb() })
        .catch((err) => {
          console.log('\n')
    ...
    if (this.config.deleteAll) {
      return co(function* (){
        ...
        if (result.objects) {
          ...
          result = yield this.store.deleteMulti(result, {
            quiet: true
          });
        }
        if(result) { return Promise.resolve(compilation);}
        return Promise.reject(new Error('delete old files failed!'));
      }.bind(this))
    }
      ...
      return Promise.all(files.map((file, index, arr) => {
        return this.uploadFile(file.name, file)
          .then((result) => {
            ...
            this.logInfo(`upload file success: ${file.name}`)
            if (files.length === uploadIndex) {
              this.logInfo(green('OSS upload finish\n'))
            }
